...
| Google agent framework | Pending | Google ADK target in [stack ADR](adr/0002-architecture-and-google-stack.md); target workflow source/tests | ADK graph executing the live workflow |
...
| Meaningful autonomous workflow | Verified locally / cloud pending | Authenticated scheduled endpoint executes the same idempotent audit and can route one issue | Scheduled cloud trigger-to-evidence-to-real-action continuous run |
...
| Architectural Discipline & Tech Stack | 30% | Typed ADK graph; deterministic authority; checkpointed state; least privilege; idempotency; isolation and fallback | Source links, tests, IAM policy, duplicate-event proof, red/green receipt, trace with one run ID |
...
| E5 | Baseline exact failure and candidate green result in separate clean verifiers | Signed/hashed verifier receipt | Pending |
| E6 | Exact tested tree published as one draft PR | GitHub PR, commit/tree SHA, hidden action marker | Pending |
...
| E9 | Unsafe patch or missing evidence fails closed | PR remains blocked with `NOT_RUN`; missing evidence abstains | Verified locally / isolation pending |
...
- Confirm every entrant's eligibility and accepted Devpost team membership; entrant
  status is intentionally not inferred here.
